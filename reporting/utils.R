...
insert.rules.to.tabular = function(n_rows, include.colnames = TRUE){
    out = list()
    out$pos = as.list(-1:n_rows)
    if (include.colnames){
        out$command = c('\\toprule ', rep('\\midrule ', n_rows), '\\bottomrule\n')
    } else {
        out$command = c('\\toprule ', '', rep('\\midrule ', n_rows - 1), '\\bottomrule\n')
    }
    return(out)
}
...
options(
    ...
    xtable.include.colnames = TRUE,
    xtable.NA.string="-",
    xtable.sanitize.text.function = sanitize.tex,
    xtable.sanitize.rownames.function = bold.tex,
    xtable.sanitize.colnames.function = bold.tex
)
...
get.calibration.plot.data = function(model, data, model_type = "final", cv_metric = "train", fold = 0, n_bins = 10, discrete_flag = FALSE){
    ...
    out = data.to.XY(data, cv_metric, fold)
    predictions = get.predictions(out$X, model)
    calib_df = get.calibration.hist.df(scores = predictions$scores,
