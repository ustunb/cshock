...
#all positive weights (note: negative weight = 1.0)
all_positive_weights=(1 2 3 4 5 10 20 30 40 50)

#method choice
R_run_cart="TRUE"
R_run_c50_rule="FALSE"
R_run_c50_tree="TRUE"
R_run_lars_lasso="FALSE"
R_run_lars_ridge="TRUE"
R_run_lars_elasticnet="FALSE"
...
for w_pos in ${all_positive_weights[*]}; do
    ...
    writeSettingsToDisk "${run_dir}${run_name}" "CP_"
    writeSettingsToDisk "${run_dir}${run_name}" "R_"

    #R needs to be run from home_dir
    cd "${home_dir}" 
    Rscript "${code_dir}train_models.R" "${train_name}" "${run_name}" "${run_log}"
