...
bool_df = df %>%
    mutate(#
        ...
        #
        # hr mean replacements (12-15)
        hr_min_geq_100 = hr_min >= 100,
        hr_max_leq_100 = hr_max <= 100,
        ...
        sys_bp_min_geq_160 = sys_bp_min >= 160, #1-13
        sys_bp_min_geq_180 = sys_bp_min >= 180, #1-13
        #
        sys_bp_max_leq_80 = sys_bp_max <= 80,
        #
        dias_bp_min_geq_90 = dias_bp_min >= 90, #1-13
        dias_bp_min_geq_110 = dias_bp_min >= 110, #1-13
        #
        ...
        #
        sp_o2_max_leq_88 = sp_o2_max <= 88, #1-13
        sp_o2_min_geq_95 = sp_o2_min >= 95,
        #
        #sp_o2_mean_geq_95 = sp_o2_mean >= 95, #Q1
...
if (version_code == 7){
    bool_df = bool_df %>% filter(vent == TRUE)
    cts_df = cts_df %>% filter(vent == TRUE)
}

if (version_code == 8){
    bool_df = bool_df %>% filter(vent == FALSE)
    cts_df = cts_df %>% filter(vent == FALSE)
}


if (version_code %in% c(6, 7, 8)){

    bool_df = bool_df %>%
        select(-hr_mean_geq_100, -resp_rate_mean_leq_20, -vent)

    cts_df = cts_df %>%
        select(-hr_mean, -resp_rate_mean, -vent)
}


if (version_code %in% c(9)){

    bool_df = bool_df %>%
        select(-hr_mean_geq_100, -resp_rate_mean_leq_20)

    cts_df = cts_df %>%
        select(-hr_mean, -resp_rate_mean)
}
