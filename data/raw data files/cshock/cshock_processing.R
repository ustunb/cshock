...
bool_df = df %>%
    mutate(#
        ...
        #
        #sys_bp_min_leq_80 = sys_bp_min <= 80, #2-1
        sys_bp_min_lt_80 = sys_bp_min < 80, #2-1
        sys_bp_min_geq_140 = sys_bp_min >= 140, #1-13
        #sys_bp_min_geq_150 = sys_bp_min >= 150, #1-13
        ...
        #sp_o2_min_geq_88 = sp_o2_min >= 88, #2-1
...
# versioning
if (version_code == 11){
    bool_df = bool_df %>% select(-vent)
    cts_df = cts_df %>% select(-vent)
}

if (version_code == 12){
    bool_df = bool_df %>% filter(vent == TRUE) %>% select(-vent)
    cts_df = cts_df %>% filter(vent == TRUE) %>% select(-vent)
}

if (version_code == 13){
    bool_df = bool_df %>% filter(vent == FALSE) %>% select(-vent)
    cts_df = cts_df %>% filter(vent == FALSE) %>% select(-vent)
}
