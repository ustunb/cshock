...
# output file names
results_name = get_results_name(input['data_name'], input['method_name'], input['fold_id'])
...
df_cv = pd.concat([fit_glmnet_cv(data, cvindices, nlambda = 100, fold_id = input['fold_id'], alpha = a) for a in alpha_values]).reset_index(drop = True)
...
report_pdf = make_report(template_file = reporting_dir / 'baseline.Rmd',
